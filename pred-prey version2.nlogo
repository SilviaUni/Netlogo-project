;two types of breed - predator and prey
...
;set up the landscape and the breeds on the screen initially
to setup
  cp
  cd
  no-display
...
;set up a landscape
to setup-landscape
  ;the turtles precedently create are hidden from the black screen if they are situated in the center of the black screen, it's for the red sentence "sorry, you are dead" can be clearly visible.
...
;set up predators initially 20 and random positions, color black
to setup-predators
  set-default-shape predators "wolf"
  create-predators 20
  ask predators [setxy random-xcor random-ycor
    set color black]
end

;set up preys initially 10 and random positions, white color
to setup-prey
  set-default-shape prey "sheep"
  create-prey 10
  ask prey [setxy random-xcor random-ycor
    set color 9.9]
end
...
486
268
Display to the user what happend after the simulation ran...
15
0.0
0
