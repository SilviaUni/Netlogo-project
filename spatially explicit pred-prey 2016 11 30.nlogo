...
;run the following code in -GO-
   foreach sort predators [ask prey [set dist distance ?]
                           ask prey with-min [dist][ifelse dist < search-radius *  15
                                                          [ifelse dist < catch-dist  * 15
                                                                [set color red
                                                                ask ? [set RPenergy RPenergy + 1]]
                                                                [ask ? [face one-of prey with-min [dist]
   ...
   foreach sort prey [ask predators [set dist distance ?]  
                      ask predators with-min [dist] [ifelse dist < prey-dodge-radius * 4 + 11
                                                             [ask ? [set heading [heading] of one-of predators with-min [dist] lt random 35 rt random 35  fd Nspeed ]]
                                                             [ask ? [right random 50
                                                                     left random  50
                                                                     forward Nspeed]]]
                      ask prey with [distance ? < 100] [ifelse prey-dd = 0
                                                             []
                                                             [ifelse (distance ? < 10) and (RNenergy >=  1)
                                                                      [ask ? [ set RNenergy RNenergy - 0.05 * (1 + prey-dd)]]
                                                                      [ifelse (distance ? < 30) and (RNenergy  >=  1 )
                                                                            [ask ? [ set RNenergy RNenergy - 0.0025 *(1 + prey-dd)]]
                                                                            [ifelse (distance ? < 50) and (RNenergy   >=  1)
                                                                                  [ask ? [ set RNenergy RNenergy - 0.0017 *(1 + prey-dd)]]
                                                                                  [ifelse (distance ? < 100) and (RNenergy   >=  1)
                                                                                        [ask ? [ set RNenergy RNenergy - 0.0013 *(1 + prey-dd)]]
                                                                                        [set size 11 ]]]]]]
                      ask ? [set RNenergy RNenergy + 0.1
                             set N-age N-age + 1
                             set size 11 + (RNenergy  * Nfactorsize) ]]
...
to GO-death 
;run the following code in -GO- 
    foreach sort prey [ask ? [if (color = red) [ask ? [set prey-dead  prey-dead +  1
                                                                      die]]]]
   ...
   foreach sort predators[ask ? [if  random-float 101 < predators-probability-die [ask ? [set predators-dead predators-dead + 1
           die]]]]
...
to GO-Reproduction
;run the following code in -GO-
   foreach sort predators [if [predators-energy_to-reproduce] of ? < [RPenergy] of ? [ask ? [hatch 1 [set P-age 0
                                                                                                      set RPenergy 1
                                                                                                      set Pborn Pborn + 1]
                                                                                      set RPenergy 1]]]
   foreach sort prey [if [prey-energy_to-reproduce ] of ? < [RNenergy] of ? [ask ? [hatch 1 [set N-age 0
                                                                                             set RNenergy 1
                                                                                             set Nborn Nborn + 1
                                                                                             set label ""
                                                                                             set label-color 33]
